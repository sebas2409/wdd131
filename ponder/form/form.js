...
form.addEventListener("submit", function (event) {
    ...
    if (type === "one" && selectedCampuses.length === 0) {
        output.textContent = "Please select at least one campus if you choose the 'one' preference.";
        return;
    }
    ...
    if (type === "many" && note.length === 0) {
        output.textContent = "Please explain why you need to travel to many campuses in the notes field.";
        return;
    }
    ...
    if (type === "many" && selectedCampuses.length < 2) {
        output.textContent = "Please select at least two campuses if you choose the 'many' preference.";
        return;
    }
